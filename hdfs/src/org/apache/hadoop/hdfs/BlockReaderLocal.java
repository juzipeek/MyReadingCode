...
class BlockReaderLocal extends FSInputChecker implements BlockReader
{
	...
	private static class LocalDatanodeInfo
	{
		...
		LocalDatanodeInfo()
		{
			...
			cache = Collections.synchronizedMap(new LinkedHashMap<Block, BlockLocalPathInfo>(hashTableCapacity, hashTableLoadFactor, true)
			{
		...
		private synchronized ClientDatanodeProtocol getDatanodeProxy(DatanodeInfo node, Configuration conf, int socketTimeout, boolean connectToDnViaHostname) throws IOException
		{
			if (proxy == null)
			{
				proxy = DFSClient.createClientDatanodeProtocolProxy(node, conf, socketTimeout, connectToDnViaHostname);
			}
			return proxy;
		}
	 ...
	 */
	static BlockReaderLocal newBlockReader(Configuration conf, String file, Block blk, Token<BlockTokenIdentifier> token, DatanodeInfo node, int socketTimeout, long startOffset,
	                                       long length, boolean connectToDnViaHostname) throws IOException
	{
		...
		try
		{
			...
			{
				LOG.debug("New BlockReaderLocal for file " + blkfile + " of size " + blkfile.length() + " startOffset " + startOffset + " length " + length +
						" short circuit checksum " + skipChecksum);
			}
	...
	private static BlockLocalPathInfo getBlockPathInfo(Block blk, DatanodeInfo node, Configuration conf, int timeout, Token<BlockTokenIdentifier> token,
	...
	{
		return conf.getBoolean(DFSConfigKeys.DFS_CLIENT_READ_SHORTCIRCUIT_SKIP_CHECKSUM_KEY, DFSConfigKeys.DFS_CLIENT_READ_SHORTCIRCUIT_SKIP_CHECKSUM_DEFAULT);
	}

	private BlockReaderLocal(Configuration conf, String hdfsfile, Block block, Token<BlockTokenIdentifier> token, long startOffset, long length, BlockLocalPathInfo pathinfo,
	                         FileInputStream dataIn) throws IOException
	{
	...
	private BlockReaderLocal(Configuration conf, String hdfsfile, Block block, Token<BlockTokenIdentifier> token, long startOffset, long length, BlockLocalPathInfo pathinfo,
	                         DataChecksum checksum, boolean verifyChecksum, FileInputStream dataIn, FileInputStream checksumIn) throws IOException
	{
		super(new Path("/blk_" + block.getBlockId() + ":of:" + hdfsfile) /*too non path-like?*/, 1, verifyChecksum, checksum.getChecksumSize() > 0 ? checksum : null,
     ...
     */
		...
		if (startOffset < 0 || startOffset > blockLength || (length + startOffset) > blockLength)
		{
	...
	protected synchronized int readChunk(long pos, byte[] buf, int offset, int len, byte[] checksumBuf) throws IOException
	{
		...
		{
			throw new IOException("Cannot read checksum into buffer. " + "The buffer must be exactly '" + checksumSize + "' bytes long to hold the checksum bytes.");
		}

		if ((pos + firstChunkOffset) != lastChunkOffset)
		{
			throw new IOException("Mismatch in pos : " + pos + " + " + firstChunkOffset + " != " + lastChunkOffset);
		}
