...
class LeaseRenewer
{
	...
	private void unsyncSetGraceSleepPeriod(final long gracePeriod)
	{
		if (gracePeriod < 100L)
		{
			throw new IllegalArgumentException(gracePeriod + " = gracePeriod < 100ms is too small.");
		}
	...
	{
		return emptyTime != Long.MAX_VALUE && System.currentTimeMillis() - emptyTime > gracePeriod;
	}
	...
	{
		for (long lastRenewed = System.currentTimeMillis(); !Thread.interrupted(); Thread.sleep(getSleepPeriod()))
		{
	...
	public String toString()
	{
		...
		{
			return s + ", clients=" + clientsString() + ", " + StringUtils.stringifyException(new Throwable("for testing"));
		}
		return s;
	}
	...
	private synchronized String clientsString()
	{
		...
		{
			final StringBuilder b = new StringBuilder("[").append(dfsclients.get(0).clientName);
