...
public class HftpFileSystem extends FileSystem implements DelegationTokenRenewer.Renewable
{
	...
	{
		return DelegationTokenSelector.selectHdfsDelegationToken(nnAddr, ugi, getConf());
	}
	...
	public synchronized Token<?> getDelegationToken(final String renewer) throws IOException
	{
		...
		try
		{
			c = ugi.doAs(new PrivilegedExceptionAction<Credentials>()
			{
				...
				{
					return DelegationTokenFetcher.getDTfromRemote(getUnderlyingProtocol(), nnAddr, renewer, getConf());
				}
			});
		} catch (Exception e)
		{
	...
	static Void throwIOExceptionFromConnection(final HttpURLConnection connection, final IOException ioe) throws IOException
	{
	...
	protected HttpURLConnection openConnection(String path, String query) throws IOException
	{
		try
		{
			...
			final URL url = new URI(getUnderlyingProtocol(), null, nnAddr.getHostName(), nnAddr.getPort(), path, query, null).toURL();
	...
	{
		final HttpURLConnection connection = openConnection("/data" + f.makeQualified(this).toUri().getPath(), "ugi=" + getUgiParameter());
		...
		return new FSDataInputStream(new FSInputStream()
		{
			...
			private void update(final boolean isEOF, final int n) throws IOException
			{
				...
				else
				{
					...
					{
						throw new IOException("Got EOF but byteread = " + currentPos + " < filelength = " + filelength);
					}
				}
			}
	...
	class LsParser extends DefaultHandler
	{
		...
		public void startElement(String ns, String localname, String qname, Attributes attrs) throws SAXException
		{
			...
			FileStatus fs = "file".equals(qname) ?
					new FileStatus(Long.valueOf(attrs.getValue("size")).longValue(), false, Short.valueOf(attrs.getValue("replication")).shortValue(),
							Long.valueOf(attrs.getValue("blocksize")).longValue(), modif, atime, FsPermission.valueOf(attrs.getValue("permission")), attrs.getValue("owner"),
							attrs.getValue("group"), new Path(getUri().toString(), attrs.getValue("path")).makeQualified(HftpFileSystem.this)) :
					new FileStatus(0L, true, 0, 0L, modif, atime, FsPermission.valueOf(attrs.getValue("permission")), attrs.getValue("owner"), attrs.getValue("group"),
							new Path(getUri().toString(), attrs.getValue("path")).makeQualified(HftpFileSystem.this));
			fslist.add(fs);
		}
		...
		private void fetchList(String path, boolean recur) throws IOException
		{
			try
			{
				...
				HttpURLConnection connection = openConnection("/listPaths" + path, "ugi=" + getUgiParameter() + (recur ? "&recursive=yes" : ""));
				connection.setRequestMethod("GET");
				connection.connect();
	...
	private class ChecksumParser extends DefaultHandler
	{
		...
		{
			final HttpURLConnection connection = openConnection("/fileChecksum" + f, "ugi=" + getUgiParameter());
			try
			{
   ...
   */
	...
	private class ContentSummaryParser extends DefaultHandler
	{
		...
		{
			final HttpURLConnection connection = openConnection("/contentSummary" + path, "ugi=" + getUgiParameter());
			InputStream in = null;
			try
			{
				in = connection.getInputStream();
	...
	private static ContentSummary toContentSummary(Attributes attrs) throws SAXException
	{
		...
		try
		{
			return new ContentSummary(Long.parseLong(length), Long.parseLong(fileCount), Long.parseLong(directoryCount), Long.parseLong(quota), Long.parseLong(spaceConsumed),
					Long.parseLong(spaceQuota));
		} catch (Exception e)
		{
			throw new SAXException(
					"Invalid attributes: length=" + length + ", fileCount=" + fileCount + ", directoryCount=" + directoryCount + ", quota=" + quota + ", spaceConsumed=" +
	...
	public static class TokenManager extends TokenRenewer
	{
		...
		public long renew(Token<?> token, Configuration conf) throws IOException
		{
			...
			return DelegationTokenFetcher.
					renewDelegationToken(getUnderlyingProtocol(), serviceAddr, (Token<DelegationTokenIdentifier>) token, conf);
		}
		...
		public void cancel(Token<?> token, Configuration conf) throws IOException
		{
			...
			DelegationTokenFetcher.
					cancelDelegationToken(getUnderlyingProtocol(), serviceAddr, (Token<DelegationTokenIdentifier>) token, conf);
		}
