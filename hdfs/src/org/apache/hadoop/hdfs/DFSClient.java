...
public class DFSClient implements FSConstants, java.io.Closeable
{
	...
	{
		return createNamenode(createRPCNamenode(nameNodeAddr, conf, UserGroupInformation.getCurrentUser()), conf);
	}
	...
	{
		return (ClientProtocol) RPC.getProxy(ClientProtocol.class, ClientProtocol.versionID, nameNodeAddr, ugi, conf, NetUtils.getSocketFactory(conf, ClientProtocol.class), 0,
				RetryUtils.getMultipleLinearRandomRetry(conf, DFSConfigKeys.DFS_CLIENT_RETRY_POLICY_ENABLED_KEY, DFSConfigKeys.DFS_CLIENT_RETRY_POLICY_ENABLED_DEFAULT,
						DFSConfigKeys.DFS_CLIENT_RETRY_POLICY_SPEC_KEY, DFSConfigKeys.DFS_CLIENT_RETRY_POLICY_SPEC_DEFAULT), false);
	}

	private static ClientProtocol createNamenode(ClientProtocol rpcNamenode, Configuration conf) throws IOException
	{
		//default policy
		@SuppressWarnings("unchecked") final RetryPolicy defaultPolicy = RetryUtils.getDefaultRetryPolicy(conf, DFSConfigKeys.DFS_CLIENT_RETRY_POLICY_ENABLED_KEY,
				DFSConfigKeys.DFS_CLIENT_RETRY_POLICY_ENABLED_DEFAULT, DFSConfigKeys.DFS_CLIENT_RETRY_POLICY_SPEC_KEY, DFSConfigKeys.DFS_CLIENT_RETRY_POLICY_SPEC_DEFAULT,
				SafeModeException.class);

		//create policy
		RetryPolicy createPolicy = RetryPolicies.retryUpToMaximumCountWithFixedSleep(5, LEASE_SOFTLIMIT_PERIOD, TimeUnit.MILLISECONDS);
	...
	static ClientDatanodeProtocol createClientDatanodeProtocolProxy(DatanodeInfo di, Configuration conf, Block block, Token<BlockTokenIdentifier> token, int socketTimeout,
	                                                                boolean connectToDnViaHostname) throws IOException
	{
		...
		return (ClientDatanodeProtocol) RPC.getProxy(ClientDatanodeProtocol.class, ClientDatanodeProtocol.versionID, addr, ticket, conf, NetUtils.getDefaultSocketFactory(conf),
				socketTimeout);
	}
	...
	static ClientDatanodeProtocol createClientDatanodeProtocolProxy(DatanodeInfo di, Configuration conf, int socketTimeout, boolean connectToDnViaHostname) throws IOException
	{
		...
		return (ClientDatanodeProtocol) RPC.getProxy(ClientDatanodeProtocol.class, ClientDatanodeProtocol.versionID, addr, conf, NetUtils.getDefaultSocketFactory(conf),
				socketTimeout);
	}
	...
	DFSClient(InetSocketAddress nameNodeAddr, ClientProtocol rpcNamenode, Configuration conf, FileSystem.Statistics stats) throws IOException
	{
		...
		else
		{
			...
			else
			{
				throw new IllegalArgumentException(
						"Expecting exactly one of nameNodeAddr and rpcNamenode being null: " + "nameNodeAddr=" + nameNodeAddr + ", rpcNamenode=" + rpcNamenode);
			}
		}
		...
		this.connectToDnViaHostname = conf.getBoolean(DFSConfigKeys.DFS_CLIENT_USE_DN_HOSTNAME, DFSConfigKeys.DFS_CLIENT_USE_DN_HOSTNAME_DEFAULT);
		if (LOG.isDebugEnabled())
		{
			LOG.debug("Connect to datanode via hostname is " + connectToDnViaHostname);
		}
		String localInterfaces[] = conf.getStrings(DFSConfigKeys.DFS_CLIENT_LOCAL_INTERFACES);
	 ...
	 */
	private BlockReader getLocalBlockReader(Configuration conf, String src, Block blk, Token<BlockTokenIdentifier> accessToken, DatanodeInfo chosenNode, int socketTimeout,
	                                        long offsetIntoBlock) throws InvalidToken, IOException
	{
		try
		{
			return BlockReaderLocal.newBlockReader(conf, src, blk, accessToken, chosenNode, socketTimeout, offsetIntoBlock, blk.getNumBytes() - offsetIntoBlock,
	...
	{
		return create(src, overwrite, replication, blockSize, progress, conf.getInt("io.file.buffer.size", 4096));
	}
	...
	{
		return create(src, FsPermission.getDefault(), overwrite, replication, blockSize, progress, buffersize);
	}
	...
	{
		return create(src, permission, overwrite, true, replication, blockSize, progress, buffersize);
	}
	 ...
	 */
	public OutputStream create(String src, FsPermission permission, boolean overwrite, boolean createParent, short replication, long blockSize, Progressable progress,
	                           int buffersize) throws IOException
	{
		...
		final DFSOutputStream result = new DFSOutputStream(src, masked, overwrite, createParent, replication, blockSize, progress, buffersize,
	 ...
	 */
	public FSDataOutputStream append(final String src, final int buffersize, final Progressable progress, final FileSystem.Statistics statistics) throws IOException
	{
	...
	private DFSOutputStream append(String src, int buffersize, Progressable progress) throws IOException
	{
		...
		final DFSOutputStream result = new DFSOutputStream(src, buffersize, progress, lastBlock, stat, conf.getInt("io.bytes.per.checksum", 512));
		beginFileLease(src, result);
		return result;
	}
			...
			throws IOException
	{
		...
		for (int i = 0; i < locatedblocks.size(); i++)
		{
			...
			final int timeout = (socketTimeout > 0) ? (socketTimeout + HdfsConstants.READ_TIMEOUT_EXTENSION * datanodes.length) : 0;
			...
			for (int j = 0; !done && j < datanodes.length; j++)
			{
				...
				try
				{
					...
					if (LOG.isDebugEnabled())
					{
						LOG.debug("write to " + dnName + ": " + DataTransferProtocol.OP_BLOCK_CHECKSUM + ", block=" + block);
					}
					...
					if (reply != DataTransferProtocol.OP_STATUS_SUCCESS)
					{
						if (reply == DataTransferProtocol.OP_STATUS_ERROR_ACCESS_TOKEN && i > lastRetriedIndex)
						{
							if (LOG.isDebugEnabled())
							{
								LOG.debug("Got access token error in response to OP_BLOCK_CHECKSUM " + "for file " + src + " for block " + block + " from datanode " + dnName +
						...
						{
							throw new IOException("Bad response " + reply + " for block " + block + " from datanode " + dnName);
						}
					}
	...
	void setQuota(String src, long namespaceQuota, long diskspaceQuota) throws IOException
	{
		...
		if ((namespaceQuota <= 0 && namespaceQuota != FSConstants.QUOTA_DONT_SET &&
				namespaceQuota != FSConstants.QUOTA_RESET) || (diskspaceQuota <= 0 && diskspaceQuota != FSConstants.QUOTA_DONT_SET &&
     ...
     */
       ...
       */
		...
		private RemoteBlockReader(String file, long blockId, DataInputStream in, DataChecksum checksum, boolean verifyChecksum, long startOffset, long firstChunkOffset,
		                          Socket dnSock)
		{
			super(new Path("/blk_" + blockId + ":of:" + file)/*too non path-like?*/, 1, verifyChecksum, checksum.getChecksumSize() > 0 ? checksum : null,
		...
		public static BlockReader newBlockReader(Socket sock, String file, long blockId, Token<BlockTokenIdentifier> accessToken, long genStamp, long startOffset, long len,
		                                         int bufferSize) throws IOException
		{
			return newBlockReader(sock, file, blockId, accessToken, genStamp, startOffset, len, bufferSize, true);
		}
		...
		{
			return newBlockReader(sock, file, blockId, accessToken, genStamp, startOffset, len, bufferSize, verifyChecksum, "");
		}

		public static BlockReader newBlockReader(Socket sock, String file, long blockId, Token<BlockTokenIdentifier> accessToken, long genStamp, long startOffset, long len,
		                                         int bufferSize, boolean verifyChecksum, String clientName) throws IOException
		{
			...
			if (status != DataTransferProtocol.OP_STATUS_SUCCESS)
			{
				if (status == DataTransferProtocol.OP_STATUS_ERROR_ACCESS_TOKEN)
				{
					throw new InvalidBlockTokenException(
							"Got access token error for OP_READ_BLOCK, self=" + sock.getLocalSocketAddress() + ", remote=" + sock.getRemoteSocketAddress() + ", for file " + file +
									", for block " + blockId + "_" + genStamp);
				}
				else
				{
					throw new IOException("Got error for OP_READ_BLOCK, self=" + sock.getLocalSocketAddress() + ", remote=" + sock.getRemoteSocketAddress() + ", for file " + file +
							", for block " + blockId + "_" + genStamp);
				}
			}
     ...
     */
		private void checksumOk(Socket sock)
		{
			try
			{
				...
				byte buf[] = {(DataTransferProtocol.OP_STATUS_CHECKSUM_OK >>> 8) & 0xff, (DataTransferProtocol.OP_STATUS_CHECKSUM_OK) & 0xff};
				out.write(buf);
				out.flush();
			} catch (IOException e)
			{
     ...
     * parallel accesses to DFSInputStream (through ptreads) properly */
		private ConcurrentHashMap<DatanodeInfo, DatanodeInfo> deadNodes = new ConcurrentHashMap<DatanodeInfo, DatanodeInfo>();
		...
		synchronized void openInfo() throws IOException
		{
			for (int retries = 3; retries > 0; retries--)
			{
				...
				else
				{
					...
					DFSClient.LOG.warn("Last block locations unavailable. " + "Datanodes might not have reported blocks completely." + " Will retry for " + retries + " times");
					waitFor(4000);
				}
			}
			...
			for (int i = 0; i < last.getLocations().length && newBlock == null; i++)
			{
				...
				} catch (IOException e)
				{
					if (e.getMessage().startsWith(
							"java.io.IOException: java.lang.NoSuchMethodException: " + "org.apache.hadoop.hdfs.protocol" + ".ClientDatanodeProtocol.getBlockInfo"))
					{
					...
					{
						LOG.info("Failed to get block info from " + datanode.getHostName() + " probably does not have " + last.getBlock(), e);
					}
				} finally
				{
					if (primary != null)
		...
		private boolean shouldTryShortCircuitRead(InetSocketAddress targetAddr) throws IOException
		{
			...
			return shortCircuitLocalReads && !blockUnderConstruction() && isLocalAddress(targetAddr);
		}
		...
		private synchronized DatanodeInfo blockSeekTo(long target) throws IOException
		{
			...
			while (true)
			{
				...
				if (shouldTryShortCircuitRead(targetAddr))
				{
					try
					{
						blockReader = getLocalBlockReader(conf, src, blk, accessToken, chosenNode, DFSClient.this.socketTimeout, offsetIntoBlock);
						return chosenNode;
              ...
              /* Get a new access token and retry. */
						...
						{
							LOG.info("Failed to read " + targetBlock.getBlock() + " on local machine" + StringUtils.stringifyException(ex));
				...
				try
				{
					...
					blockReader = RemoteBlockReader.newBlockReader(s, src, blk.getBlockId(), accessToken, blk.getGenerationStamp(), offsetIntoBlock,
							blk.getNumBytes() - offsetIntoBlock, buffersize, verifyChecksum, clientName);
					return chosenNode;
				} catch (IOException ex)
				{
					if (refetchToken > 0 && tokenRefetchNeeded(ex, targetAddr))
					...
					{
						LOG.warn("Failed to connect to " + targetAddr + ", add to deadNodes and continue" + ex);
						if (LOG.isDebugEnabled())
						{
		...
		private DNAddrPair chooseDataNode(LocatedBlock block) throws IOException
		{
			while (true)
			{
				...
				try
				{
					...
					InetSocketAddress targetAddr = NetUtils.createSocketAddr(chosenNode.getName(connectToDnViaHostname));
				...
				} catch (IOException ie)
				{
					...
					LOG.info("Could not obtain " + block.getBlock() + " from any node: " + ie + ". Will get new block locations from namenode and retry...");
					try
					{
						Thread.sleep(3000);
			...
			while (true)
			{
				...
				try
				{
					...
					else
					{
						...
						LOG.debug("Connecting to " + targetAddr);
						NetUtils.connect(dn, targetAddr, getRandomLocalInterfaceAddr(), socketTimeout);
						dn.setSoTimeout(socketTimeout);
						reader = RemoteBlockReader.newBlockReader(dn, src, block.getBlock().getBlockId(), accessToken, block.getBlock().getGenerationStamp(), start, len,
				...
				} catch (IOException e)
				{
					...
					{
						LOG.warn("Failed to connect to " + targetAddr + " for file " + src + " for block " + block.getBlock() + ":" + e);
						if (LOG.isDebugEnabled())
						{
			...
			if (pos <= targetPos && targetPos <= blockEnd)
			{
				...
				if (diff <= TCP_WINDOW_SIZE)
				{
					...
					{//make following read to retry
						LOG.debug("Exception while seek to " + targetPos + " from " + currentBlock + " of " + src + " from " + currentNode +
								": " + StringUtils.stringifyException(e));
					}
				}
			}
	...
	class DFSOutputStream extends FSOutputSummer implements Syncable
	{
			...
			public void run()
			{
				...
				while (!closed && clientRunning && !lastPacketInBlock)
				{
					...
					try
					{
						...
						assert seqno != PipelineAck.UNKOWN_SEQNO : "Ack for unkown seqno should be a failed ack: " + ack;
		...
		private boolean processDatanodeError(boolean hasError, boolean isAppend)
		{
			...
			if (errorIndex >= 0)
			{
				LOG.warn("Error Recovery for " + block + " bad datanode[" + errorIndex + "] " + (nodes == null ? "nodes == null" : nodes[errorIndex].getName()));
			}
		 ...
		 * @see ClientProtocol#create(String, FsPermission, String, boolean, short, long)
		 */
		DFSOutputStream(String src, FsPermission masked, boolean overwrite, boolean createParent, short replication, long blockSize, Progressable progress, int buffersize,
		                int bytesPerChecksum) throws IOException
		{
			...
			{
				throw re.unwrapRemoteException(AccessControlException.class, FileAlreadyExistsException.class, FileNotFoundException.class, NSQuotaExceededException.class,
		...
		DFSOutputStream(String src, int buffersize, Progressable progress, LocatedBlock lastBlock, HdfsFileStatus stat, int bytesPerChecksum) throws IOException
		{
			...
			if (lastBlock != null)
			{
				...
				else
				{
					...
					// smaller size packet.
					//
					computePacketChunkSize(Math.min(writePacketSize, freeInLastBlock), bytesPerChecksum);
				}
		...
		private boolean createBlockOutputStream(DatanodeInfo[] nodes, String client, boolean recoveryFlag)
		{
			...
			try
			{
				...
				s = socketFactory.createSocket();
				timeoutValue = (socketTimeout > 0) ? (3000 * nodes.length + socketTimeout) : 0;
				LOG.debug("Connecting to " + dnName);
				NetUtils.connect(s, target, getRandomLocalInterfaceAddr(), timeoutValue);
				s.setSoTimeout(timeoutValue);
				s.setSendBufferSize(DEFAULT_DATA_SOCKET_SIZE);
				LOG.debug("Send buf size " + s.getSendBufferSize());
				long writeTimeout = (datanodeWriteTimeout > 0) ? (HdfsConstants.WRITE_TIMEOUT_EXTENSION * nodes.length + datanodeWriteTimeout) : 0;
		...
		private LocatedBlock locateFollowingBlock(long start, DatanodeInfo[] excludedNodes) throws IOException
		{
			...
			while (true)
			{
				...
				while (true)
				{
					...
					{
						IOException ue = e.unwrapRemoteException(FileNotFoundException.class, AccessControlException.class, NSQuotaExceededException.class,
								DSQuotaExceededException.class);
						if (ue != e)
						{
						...
						if (e.getMessage().startsWith("java.io.IOException: java.lang.NoSuchMethodException: " +
		...
		synchronized void abort() throws IOException
		{
			...
			setLastException(new IOException("Lease timeout of " + (hdfsTimeout / 1000) + " seconds expired."));
			closeThreads();
			endFileLease(src);
		}
		...
		synchronized void setChunksPerPacket(int value)
		{
			...
			packetSize = DataNode.PKT_HEADER_LEN + SIZE_OF_INTEGER +
					(checksum.getBytesPerChecksum() + checksum.getChecksumSize()) * chunksPerPacket;
		}
