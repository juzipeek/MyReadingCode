...
public class DelegationTokenFetcher
{
		...
		else
		{
			...
			else
			{
				if (webUrl != null)
				{
					...
					getDTfromRemote(uri.getScheme(), new InetSocketAddress(uri.getHost(), uri.getPort()), null, conf).
	 ...
	 */
	static public Credentials getDTfromRemote(String protocol, final InetSocketAddress nnAddr, String renewer, Configuration conf) throws IOException
	{
	...
	protected static String getRenewAddress(String protocol, InetSocketAddress addr, Configuration conf)
	{
		...
		{
			protocol = "https";
			int port = conf.getInt(DFSConfigKeys.DFS_NAMENODE_HTTPS_PORT_KEY, DFSConfigKeys.DFS_NAMENODE_HTTPS_PORT_DEFAULT);
	 ...
	 * @throws IOException
	 */
	static public long renewDelegationToken(String protocol, InetSocketAddress addr, Token<DelegationTokenIdentifier> tok, Configuration conf) throws IOException
	{
	...
	static private IOException getExceptionFromResponse(String resp)
	{
		...
		if (exceptionClass != null && !exceptionClass.isEmpty())
		{
			...
			{
				if (exceptionClass.contains("AccessControlException"))
				{
	 ...
	 * @throws IOException
	 */
	static public void cancelDelegationToken(String protocol, InetSocketAddress addr, Token<DelegationTokenIdentifier> tok, Configuration conf) throws IOException
	{
		final String renewAddress = getRenewAddress(protocol, addr, conf);
