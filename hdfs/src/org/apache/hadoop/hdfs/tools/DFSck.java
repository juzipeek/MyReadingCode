		...
		try
		{
			return UserGroupInformation.getCurrentUser().doAs(new PrivilegedExceptionAction<Integer>()
			{
				...
				public Integer run() throws Exception
				{
					...
					final StringBuffer url = new StringBuffer(NameNode.getHttpUriScheme() + "://");
					...
					{
						if (lastLine.endsWith(NamenodeFsck.CORRUPT_STATUS))
						{
							errCode = 1;
						}
						else
						{
							if (lastLine.endsWith(NamenodeFsck.NONEXISTENT_STATUS))
							{
								errCode = 0;
							}
						}
