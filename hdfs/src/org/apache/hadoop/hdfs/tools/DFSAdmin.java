...
public class DFSAdmin extends FsShell
{
	...
	private static class SetQuotaCommand extends DFSAdminCommand
	{
		...
		private static final String DESCRIPTION = "-setQuota <quota> <dirname>...<dirname>: " +
	...
	public void setSafeMode(String[] argv, int idx) throws IOException
	{
		...
		{
			if ("enter".equalsIgnoreCase(argv[idx]))
			{
				action = FSConstants.SafeModeAction.SAFEMODE_ENTER;
			}
			else
			{
				if ("get".equalsIgnoreCase(argv[idx]))
				{
					action = FSConstants.SafeModeAction.SAFEMODE_GET;
				}
				else
				{
					if ("wait".equalsIgnoreCase(argv[idx]))
					{
						action = FSConstants.SafeModeAction.SAFEMODE_GET;
						waitExitSafe = true;
					}
					...
					else
					{
						...
						else
						{
							...
							else
							{
								...
								{
									if (SetQuotaCommand.matches("-" + cmd))
									{
									...
									else
									{
										...
										{
											if (SetSpaceQuotaCommand.matches("-" + cmd))
											{
												System.out.println(SetSpaceQuotaCommand.DESCRIPTION);
											}
											else
											{
												if (ClearSpaceQuotaCommand.matches("-" + cmd))
												{
	...
	public int upgradeProgress(String[] argv, int idx) throws IOException
	{
		...
		{
			if ("details".equalsIgnoreCase(argv[idx]))
			{
				action = UpgradeAction.DETAILED_STATUS;
			}
			else
			{
				if ("force".equalsIgnoreCase(argv[idx]))
				{
	...
	public int refreshServiceAcl() throws IOException
	{
		...
		RefreshAuthorizationPolicyProtocol refreshProtocol = (RefreshAuthorizationPolicyProtocol) RPC.getProxy(RefreshAuthorizationPolicyProtocol.class,
				RefreshAuthorizationPolicyProtocol.versionID, NameNode.getAddress(conf), getUGI(), conf, NetUtils.getSocketFactory(conf, RefreshAuthorizationPolicyProtocol.class));
	...
	public int refreshUserToGroupsMappings() throws IOException
	{
		...
		RefreshUserMappingsProtocol refreshProtocol = (RefreshUserMappingsProtocol) RPC.getProxy(RefreshUserMappingsProtocol.class, RefreshUserMappingsProtocol.versionID,
				NameNode.getAddress(conf), getUGI(), conf, NetUtils.getSocketFactory(conf, RefreshUserMappingsProtocol.class));
