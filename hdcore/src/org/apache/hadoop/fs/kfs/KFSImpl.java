...
class KFSImpl implements IFSImpl
{
	...
	public FileStatus[] readdirplus(Path path) throws IOException
	{
		...
		for (int i = 0; i < fattr.length; i++)
		{
			...
			{
				fstatus[j] = new FileStatus(fattr[i].filesize, fattr[i].isDirectory, fattr[i].replication, (long) (1 << 26), fattr[i].modificationTime, fn);
			}
	...
	{
		return new FSDataOutputStream(new KFSOutputStream(kfsAccess, path, replication), statistics);
	}

	public FSDataInputStream open(String path, int bufferSize) throws IOException
	{
		return new FSDataInputStream(new KFSInputStream(kfsAccess, path, statistics));
	}
}
