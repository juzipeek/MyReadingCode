...
public class S3FileSystem extends FileSystem
{
	...
	private static FileSystemStore createDefaultStore(Configuration conf)
	{
		...
		RetryPolicy basePolicy = RetryPolicies.retryUpToMaximumCountWithFixedSleep(conf.getInt("fs.s3.maxRetries", 4), conf.getLong("fs.s3.sleepTimeSeconds", 10),
				TimeUnit.SECONDS);
	...
	public FSDataInputStream open(Path path, int bufferSize) throws IOException
	{
		...
		return new FSDataInputStream(new S3InputStream(getConf(), store, inode, statistics));
	}
