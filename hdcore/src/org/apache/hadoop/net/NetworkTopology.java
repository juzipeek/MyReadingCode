...
public class NetworkTopology
{
	...
	static class InnerNode extends NodeBase
	{
		...
		{
			return getPath(this).equals(NodeBase.PATH_SEPARATOR_STR) || (n.getNetworkLocation() + NodeBase.PATH_SEPARATOR_STR).
		...
		boolean add(Node n)
		{
			if (!isAncestor(n))
			{
				throw new IllegalArgumentException(n.getName() + ", which is located at " + n.getNetworkLocation() + ", is not a decendent of " + getPath(this));
			}
			if (isParent(n))
			{
	...
	{
		return (NetworkTopology) ReflectionUtils.newInstance(conf.getClass("net.topology.impl", NetworkTopology.class, NetworkTopology.class), conf);
	}
	...
	public void pseudoSortByDistance(Node reader, Node[] nodes)
	{
		...
		if (reader != null)
		{
			...
			for (int i = 0; i < nodes.length; i++)
			{
				...
				{
					if (localRackNode == -1 && isOnSameRack(reader, nodes[i]))
					{
						//local rack
						localRackNode = i;
						if (tempIndex != 0)
