...
public class NetworkTopologyWithNodeGroup extends NetworkTopology
{
	...
	public void pseudoSortByDistance(Node reader, Node[] nodes)
	{
		...
		if (reader != null)
		{
			...
			for (int i = 0; i < nodes.length; i++)
			{
				...
				{
					if (localNodeGroupNode == -1 && isOnSameNodeGroup(reader, nodes[i]))
					{
						//local node group
						localNodeGroupNode = i;
						// node local and rack local are already found
						if (tempIndex != 0 && localRackNode != -1)
						{
							break;
						}
					}
					else
					{
						...
						{
							localRackNode = i;
							if (tempIndex != 0 && localNodeGroupNode != -1)
							{
								break;
							}
						}
	...
	{
		public InnerNodeWithNodeGroup(String name, String location, InnerNode parent, int level)
		{
		...
		{
			return new InnerNodeWithNodeGroup(parentName, getPath(this), this, this.getLevel() + 1);
		}
