...
public class JvmMetricsSource implements MetricsSource
{
	...
	{
		return ms.register(SOURCE_NAME, SOURCE_DESC, new JvmMetricsSource(processName, sessionId));
	}
	...
	{
		MetricsRecordBuilder rb = builder.addRecord(RECORD_NAME).setContext(CONTEXT).tag(PROCESSNAME_KEY, PROCESSNAME_DESC, processName).tag(SESSIONID_KEY, SESSIONID_DESC,
				sessionId);
		getMemoryUsage(rb);
		getGcUsage(rb);
		getThreadUsage(rb);
	...
	private void getMemoryUsage(MetricsRecordBuilder rb)
	{
		...
		rb.addGauge(NONHEAP_USED_KEY, NONHEAP_USED_DESC, memNonHeap.getUsed() / M).addGauge(NONHEAP_COMMITTED_KEY, NONHEAP_COMMITTED_DESC, memNonHeap.getCommitted() / M).addGauge(
				HEAP_USED_KEY, HEAP_USED_DESC, memHeap.getUsed() / M).addGauge(HEAP_COMMITTED_KEY, HEAP_COMMITTED_DESC, memHeap.getCommitted() / M);
	}
	...
	private void getGcUsage(MetricsRecordBuilder rb)
	{
		...
		rb.addCounter(GC_COUNT_KEY, GC_COUNT_DESC, count).addCounter(GC_TIME_KEY, GC_TIME_DESC, timeMillis);
	}
	...
	private void getThreadUsage(MetricsRecordBuilder rb)
	{
		...
		rb.addGauge(THREADS_NEW_KEY, THREADS_NEW_DESC, threadsNew).addGauge(THREADS_RUNNABLE_KEY, THREADS_RUNNABLE_DESC, threadsRunnable).addGauge(THREADS_BLOCKED_KEY,
				THREADS_BLOCKED_DESC, threadsBlocked).addGauge(THREADS_WAITING_KEY, THREADS_WAITING_DESC, threadsWaiting).addGauge(THREADS_TIMEDWAITING_KEY,
				THREADS_TIMEDWAITING_DESC, threadsTimedWaiting).addGauge(THREADS_TERMINATED_KEY, THREADS_TERMINATED_DESC, threadsTerminated);
	}

	private void getEventCounters(MetricsRecordBuilder rb)
	{
		rb.addCounter(LOG_FATAL_KEY, LOG_FATAL_DESC, EventCounter.getFatal()).addCounter(LOG_ERROR_KEY, LOG_ERROR_DESC, EventCounter.getError()).addCounter(LOG_WARN_KEY,
				LOG_WARN_DESC, EventCounter.getWarn()).addCounter(LOG_INFO_KEY, LOG_INFO_DESC, EventCounter.getInfo());
	}
