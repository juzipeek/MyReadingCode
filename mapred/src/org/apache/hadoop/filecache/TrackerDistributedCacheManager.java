...
public class TrackerDistributedCacheManager
{
	...
	private Map<JobID, TaskDistributedCacheManager> jobArchives = Collections.synchronizedMap(new HashMap<JobID, TaskDistributedCacheManager>());
	...
	public TrackerDistributedCacheManager(Configuration conf, TaskController controller) throws IOException
	{
		...
		this.allowedCacheSubdirs = conf.getLong("mapreduce.tasktracker.local.cache.numberdirectories", DEFAULT_CACHE_SUBDIR_LIMIT);
		double cleanupPct = conf.getFloat("mapreduce.tasktracker.cache.local.keep.pct", DEFAULT_CACHE_KEEP_AROUND_PCT);
	 ...
	 * @throws IOException
	 */
	Path getLocalCache(URI cache, Configuration conf, String subDir, FileStatus fileStatus, boolean isArchive, long confFileStamp, boolean isPublic, CacheFile file)
			throws IOException
	{
		String key;
		...
		synchronized (cachedArchives)
		{
			...
			if (lcacheStatus == null)
			{
				...
				localPath = lDirAllocator.getLocalPathForWrite(cachePath, fileStatus.getLen(), trackerConf, isPublic);
	...
	{
		long size = downloadCacheObject(conf, cache, cacheStatus.localizedLoadPath, confFileStamp, isArchive, PUBLIC_CACHE_OBJECT_PERM);
	...
	{
		final String[] archiveStrings = conf.getStrings(DistributedCache.CACHE_ARCHIVES);
			...
			for (String fileString : fileStrings)
			{
				...
				if (archivesSet.contains(thisSubject))
				{
					throw new InvalidJobConfException(
							"The core URI, \"" + thisSubject + "\" is listed both in " + DistributedCache.CACHE_FILES + " and in " + DistributedCache.CACHE_ARCHIVES + " .");
				}
			}
		}
	}
	...
	protected class CleanupThread extends Thread
	{
		...
		{
			cleanUpCheckPeriod = conf.getLong("mapreduce.tasktracker.distributedcache.checkperiod", cleanUpCheckPeriod);
		}
		...
		{
			LOG.fatal("Distributed Cache cleanup thread received runtime exception." + " Exiting the TaskTracker", t);
			...
			for (CacheStatus cacheStatus : toBeDeletedCache)
			{
				...
				try
				{
					...
					if (cacheStatus.user == null)
					{
						...
						{
							TrackerDistributedCacheManager.LOG.warn("Could not delete distributed cache empty directory " + localizedDir, e);
						}
					}
					else
					{
