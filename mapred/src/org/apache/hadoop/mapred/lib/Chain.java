...
class Chain
{
	...
	private static JobConf getChainElementConf(JobConf jobConf, String confKey)
	{
		...
		try
		{
			Stringifier<JobConf> stringifier = new DefaultStringifier<JobConf>(jobConf, JobConf.class);
	 ...
	 */
	public static <K1, V1, K2, V2> void addMapper(boolean isMap, JobConf jobConf, Class<? extends Mapper<K1, V1, K2, V2>> klass, Class<? extends K1> inputKeyClass,
	                                              Class<? extends V1> inputValueClass, Class<? extends K2> outputKeyClass, Class<? extends V2> outputValueClass, boolean byValue,
	                                              JobConf mapperConf)
	{
		...
		if (!isMap)
		{
			if (jobConf.getClass(prefix + CHAIN_REDUCER_CLASS, Reducer.class) == null)
			{
		...
		if (!isMap && index == 0)
		{
			JobConf reducerConf = getChainElementConf(jobConf, prefix + CHAIN_REDUCER_CONFIG);
		...
		else
		{
			if (index > 0)
			{
				...
				JobConf previousMapperConf = getChainElementConf(jobConf, prefix + CHAIN_MAPPER_CONFIG +
						(index - 1));
	 ...
	 * <code>JobConf(boolean loadDefaults)</code> constructor with FALSE.
	 */
	public static <K1, V1, K2, V2> void setReducer(JobConf jobConf, Class<? extends Reducer<K1, V1, K2, V2>> klass, Class<? extends K1> inputKeyClass,
	                                               Class<? extends V1> inputValueClass, Class<? extends K2> outputKeyClass, Class<? extends V2> outputValueClass, boolean byValue,
	                                               JobConf reducerConf)
	{
	...
	public void configure(JobConf jobConf)
	{
		...
		for (int i = 0; i < index; i++)
		{
			...
			JobConf mConf = getChainElementConf(jobConf, prefix + CHAIN_MAPPER_CONFIG + i);
		...
		if (klass != null)
		{
			JobConf rConf = getChainElementConf(jobConf, prefix + CHAIN_REDUCER_CONFIG);
	...
	private class ChainOutputCollector<K, V> implements OutputCollector<K, V>
	{
		...
		public void collect(K key, V value) throws IOException
		{
			if (nextMapperIndex < mappers.size())
			{
				...
				Serialization nextKeySerialization = mappersKeySerialization.get(nextMapperIndex);
				Serialization nextValueSerialization = mappersValueSerialization.get(nextMapperIndex);
				Mapper nextMapper = mappers.get(nextMapperIndex);

				// invokes next mapper in chain
				nextMapper.map(key, value, new ChainOutputCollector(nextMapperIndex, nextKeySerialization, nextValueSerialization, output, reporter), reporter);
			}
			else
			{
