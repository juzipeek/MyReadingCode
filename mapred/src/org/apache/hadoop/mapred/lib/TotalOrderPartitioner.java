...
public class TotalOrderPartitioner<K extends WritableComparable, V> implements Partitioner<K, V>
{
	...
	public void configure(JobConf job)
	{
		try
		{
			...
			{
				partitions = buildTrie((BinaryComparable[]) splitPoints, 0, splitPoints.length, new byte[0], job.getInt("total.order.partitioner.max.trie.depth", 2));
			}
			else
			{
	...
	private TrieNode buildTrie(BinaryComparable[] splits, int lower, int upper, byte[] prefix, int maxDepth)
	{
		...
		for (int ch = 0; ch < 255; ++ch)
		{
			...
			trial[depth] = (byte) ch;
			result.child[0xFF & ch] = buildTrie(splits, lower, currentBound, trial, maxDepth);
		}
		// pick up the rest
		trial[depth] = 127;
