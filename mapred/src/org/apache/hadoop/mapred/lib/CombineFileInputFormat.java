...
public abstract class CombineFileInputFormat<K, V> extends FileInputFormat<K, V>
{
	...
	private void getMoreSplits(JobConf job, Path[] paths, long maxSize, long minSizeNode, long minSizeRack, List<CombineFileSplit> splits) throws IOException
	{
		...
		for (int i = 0; i < paths.length; i++)
		{
			files[i] = new OneFileInfo(paths[i], job, rackToBlocks, blockToNodes, nodeToBlocks, rackToNodes);
	...
	private static class OneFileInfo
	{
		...
		OneFileInfo(Path path, JobConf job, HashMap<String, List<OneBlockInfo>> rackToBlocks, HashMap<OneBlockInfo, String[]> blockToNodes,
		            HashMap<String, List<OneBlockInfo>> nodeToBlocks, HashMap<String, Set<String>> rackToNodes) throws IOException
		{
			this.fileSize = 0;
			...
			else
			{
				...
				for (int i = 0; i < locations.length; i++)
				{
					...
					OneBlockInfo oneblock = new OneBlockInfo(path, locations[i].getOffset(), locations[i].getLength(), locations[i].getHosts(), locations[i].getTopologyPaths());
					blocks[i] = oneblock;
