...
class TaskInProgress {
  ...
  public boolean isOnlyCommitPending() {
    for (TaskStatus t : taskStatuses.values()) {
      if (t.getRunState() == State.COMMIT_PENDING) {
        return true;
      }
    }
    return false;
  ...
  public boolean isCommitPending(TaskAttemptID taskId) {
    ...
    return t.getRunState() ==  State.COMMIT_PENDING;
  }
  ...
  synchronized boolean updateStatus(TaskStatus status) {
    ...
    if (oldStatus != null) {
      State oldState = oldStatus.getRunState();
      State newState = status.getRunState();
      ...
      if ((newState != State.RUNNING &&
           newState != State.COMMIT_PENDING &&
           newState != State.FAILED_UNCLEAN &&
           newState != State.KILLED_UNCLEAN &&
           newState != State.UNASSIGNED) &&
          (oldState == newState)) {
      ...
      if ((newState == State.RUNNING ||
          ...
          (oldState == State.FAILED ||
           oldState == State.KILLED ||
           oldState == State.FAILED_UNCLEAN ||
           oldState == State.KILLED_UNCLEAN ||
           oldState == State.SUCCEEDED ||
           oldState == State.COMMIT_PENDING)) {
        return false;
      }
  ...
  public void incompleteSubTask(TaskAttemptID taskid, 
    ...
    if (status != null) {
      ...
      if (shouldFail != null) {
        if (status.getRunState() == State.FAILED ||
            status.getRunState() == State.KILLED) {
          taskState = (shouldFail) ? State.FAILED :
                                     State.KILLED;
        } else {
          taskState = (shouldFail) ? State.FAILED_UNCLEAN :
                                     State.KILLED_UNCLEAN;
      ...
      if (taskState != State.FAILED &&
          taskState != State.KILLED &&
          taskState != State.FAILED_UNCLEAN &&
          taskState != State.KILLED_UNCLEAN) {
        LOG.info("Task '" + taskid + "' running on '" + trackerName + 
                "' in state: '" + taskState + "' being failed!");
        status.setRunState(State.FAILED);
    ...
    if (tasks.contains(taskid)) {
      ...
      } else if (taskState == State.KILLED) {
        numKilledTasks++;
      }
    }
   ...
   * {@link State.SUCCEEDED} or {@link State.KILLED}
   * 
   * @param taskId id of the completed task-attempt
   * @param finalTaskState final {@link State} of the task-attempt
   */
  private void completedTask(TaskAttemptID taskId, State finalTaskState) {
    TaskStatus status = taskStatuses.get(taskId);
   ...
   * taskid as {@link State.KILLED}.
   */
  void alreadyCompletedTask(TaskAttemptID taskid) {
    // 'KILL' the task 
    completedTask(taskid, State.KILLED);
  ...
  public void completed(TaskAttemptID taskid) {
    ...
    //
    completedTask(taskid, State.SUCCEEDED);
  ...
  boolean killTask(TaskAttemptID taskId, boolean shouldFail) {
    ...
    if(st != null && (st.getRunState() == State.RUNNING
        || st.getRunState() == State.COMMIT_PENDING ||
        st.inTaskCleanupPhase() ||
        st.getRunState() == State.UNASSIGNED)
  ...
  void recomputeProgress() {
    if (isComplete()) {
      ...
      for (Iterator<TaskAttemptID> it = taskStatuses.keySet().iterator(); it.hasNext();) {
        ...
        TaskStatus status = taskStatuses.get(taskid);
        if (status.getRunState() == State.SUCCEEDED) {
          bestProgress = 1;
          bestState = status.getStateString();
          bestCounters = status.getCounters();
          break;
        } else if (status.getRunState() == State.COMMIT_PENDING) {
        ...
        } else if (status.getRunState() == State.RUNNING) {
  ...
  boolean isRunningTask(TaskAttemptID taskid) {
    TaskStatus status = taskStatuses.get(taskid);
    return status != null && status.getRunState() == State.RUNNING;
  }
  ...
  private class FailedRanges {
    ...
    synchronized void updateState(TaskStatus status){
      if (isTestAttempt() && 
          (status.getRunState() == State.SUCCEEDED)) {
