...
class JSPUtil
{
	 ...
	 */
	public static JobWithViewAccessCheck checkAccessAndGetJob(final JobTracker jt, JobID jobid, HttpServletRequest request, HttpServletResponse response)
	...
	public static void processButtons(HttpServletRequest request, HttpServletResponse response, final JobTracker tracker) throws IOException, InterruptedException, ServletException
	{
		...
		if (privateActionsAllowed(tracker.conf) && request.getParameter("killJobs") != null)
		{
			...
			if (jobs != null)
			{
				...
				if (notAuthorized)
				{// user is not authorized to kill some/all of jobs
	...
	public static String generateJobTable(String label, Collection<JobInProgress> jobs, int refresh, int rowId, JobConf conf) throws IOException
	{
		...
		if (jobs.size() > 0)
		{
			...
			for (Iterator<JobInProgress> it = jobs.iterator(); it.hasNext(); ++rowId)
			{
				...
				sb.append("<td id=\"job_" + rowId + "\"><a href=\"jobdetails.jsp?jobid=" + jobid + "&amp;refresh=" + refresh + "\">" + jobid + "</a></td>" + "<td id=\"started_" +
						rowId + "\">" + time + "</td>" + "<td id=\"priority_" + rowId + "\">" + jobpri + "</td>" + "<td id=\"user_" + rowId + "\">" +
						HtmlQuoting.quoteHtmlChars(profile.getUser()) +
						"</td>" + "<td id=\"name_" + rowId + "\">" + ("".equals(name) ? "&nbsp;" : name) + "</td>" + "<td>" + StringUtils.formatPercent(status.mapProgress(), 2) +
						ServletUtil.percentageGraph(status.mapProgress() * 100, 80) + "</td><td>" + desiredMaps + "</td><td>" + completedMaps + "</td><td>" +
						StringUtils.formatPercent(status.reduceProgress(), 2) + ServletUtil.percentageGraph(status.reduceProgress() * 100, 80) + "</td><td>" + desiredReduces +
						"</td><td> " + completedReduces + "</td><td>" + schedulingInfo + "</td><td>" + diagnosticInfo + "</td></tr>\n");
			}
			if (isModifiable)
			{
				sb.append("</form>\n");
		...
		else
		{
			...
			for (int i = 0; i < 100 && iterator.hasNext(); i++)
			{
				...
				sb.append("<td id=\"job_" + rowId + "\">" +
								...
								(historyFileUrl == null ? "" : "<a href=\"" + JobHistoryServer.getHistoryUrlPrefix(tracker.conf) +
								...
								"<td>" + StringUtils.formatPercent(info.status.mapProgress(), 2) + ServletUtil.percentageGraph(info.status.mapProgress() * 100, 80) +
								"</td>" +

								"<td>" + StringUtils.formatPercent(info.status.reduceProgress(), 2) + ServletUtil.percentageGraph(info.status.reduceProgress() * 100, 80) +
								"</td>" +
	 ...
	 * @throws IOException
	 */
	static JobInfo getJobInfo(Path logFile, FileSystem fs, JobConf jobConf, ACLsManager acLsManager, String user) throws IOException
	{
		...
		acLsManager.checkAccess(jobid, currentUser, jobInfo.getJobQueue(), Operation.VIEW_JOB_DETAILS, jobInfo.get(Keys.USER), jobInfo.getJobACLs().get(JobACL.VIEW_JOB));
	 ...
	 * @throws ServletException
	 */
	static JobInfo checkAccessAndGetJobInfo(HttpServletRequest request, HttpServletResponse response, final JobConf jobConf, final ACLsManager acLsManager, final FileSystem fs,
	                                        final Path logFile) throws IOException, InterruptedException, ServletException
	{
		String jobid = getJobID(logFile.getName());
		String user = request.getRemoteUser();
		JobInfo job = null;
		if (user != null)
		{
			...
			} catch (AccessControlException e)
			{
				...
				String errMsg = String.format(
						"User %s failed to view %s!<br><br>%s" + "<hr>" + "<a href=\"jobhistory.jsp\">Go back to JobHistory</a><br>" + "<a href=\"http://" + trackerAddress +
								"/jobtracker.jsp\">Go back to JobTracker</a>", user, jobid, e.getMessage());
				JSPUtil.setErrorAndForward(errMsg, request, response);
				return null;
			}
		}
