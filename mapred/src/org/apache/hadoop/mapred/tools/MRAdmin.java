...
public class MRAdmin extends Configured implements Tool
{
	...
	private int refreshAuthorizationPolicy() throws IOException
	{
		...
		// Create the client
		RefreshAuthorizationPolicyProtocol refreshProtocol = (RefreshAuthorizationPolicyProtocol) RPC.getProxy(RefreshAuthorizationPolicyProtocol.class,
				RefreshAuthorizationPolicyProtocol.versionID, JobTracker.getAddress(conf), getUGI(conf), conf,
	...
	private int refreshQueues() throws IOException
	{
		...
		// Create the client
		AdminOperationsProtocol adminOperationsProtocol = (AdminOperationsProtocol) RPC.getProxy(AdminOperationsProtocol.class, AdminOperationsProtocol.versionID,
				JobTracker.getAddress(conf), getUGI(conf), conf, NetUtils.getSocketFactory(conf, AdminOperationsProtocol.class));
	...
	private int setSafeMode(String actionString) throws IOException
	{
		...
		{
			if ("enter".equalsIgnoreCase(actionString))
			{
				action = JobTracker.SafeModeAction.SAFEMODE_ENTER;
			}
			else
			{
				if ("get".equalsIgnoreCase(actionString))
				{
					action = JobTracker.SafeModeAction.SAFEMODE_GET;
				}
				else
				{
					if ("wait".equalsIgnoreCase(actionString))
					{
						action = JobTracker.SafeModeAction.SAFEMODE_GET;
						waitExitSafe = true;
					}
	...
	public int refreshSuperUserGroupsConfiguration() throws IOException
	{
		...
		// Create the client
		RefreshUserMappingsProtocol refreshProtocol = (RefreshUserMappingsProtocol) RPC.getProxy(RefreshUserMappingsProtocol.class, RefreshUserMappingsProtocol.versionID,
				JobTracker.getAddress(conf), getUGI(conf), conf, NetUtils.getSocketFactory(conf, RefreshUserMappingsProtocol.class));
	...
	public int run(String[] args) throws Exception
	{
		...
		//
		if ("-refreshServiceAcl".equals(cmd) || "-refreshQueues".equals(cmd) || "-refreshNodes".equals(cmd) ||
